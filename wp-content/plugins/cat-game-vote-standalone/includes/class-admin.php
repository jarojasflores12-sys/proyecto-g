...
class CatGame_Admin {
    ...
    public static function settings_page(): void {
        ...
        $display_name = sanitize_text_field((string) ($settings['display_name'] ?? 'PetUnity'));
        if ($display_name === '') {
            $display_name = 'PetUnity';
        }
        $display_subtitle = sanitize_text_field((string) ($settings['display_subtitle'] ?? 'Un espacio para conectar, cuidar y celebrar mascotas.'));
        ?>
        <div class="wrap">
            ...
            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                ...
                <table class="form-table">
                    <tr>
                        <th><label for="catgame_display_name">Nombre visible del juego</label></th>
                        <td>
                            <input class="regular-text" type="text" name="display_name" id="catgame_display_name" maxlength="80" value="<?php echo esc_attr($display_name); ?>" placeholder="PetUnity" />
                            <p class="description">Se muestra en el header público del juego.</p>
                        </td>
                    </tr>
                    <tr>
                        <th><label for="catgame_display_subtitle">Subtítulo visible</label></th>
                        <td>
                            <input class="regular-text" type="text" name="display_subtitle" id="catgame_display_subtitle" maxlength="160" value="<?php echo esc_attr($display_subtitle); ?>" placeholder="Un espacio para conectar, cuidar y celebrar mascotas." />
                        </td>
                    </tr>
                    <tr>
    ...
    public static function save_settings(): void {
        ...
        $display_name = sanitize_text_field(wp_unslash($_POST['display_name'] ?? 'PetUnity'));
        if ($display_name === '') {
            $display_name = 'PetUnity';
        }
        $display_subtitle = sanitize_text_field(wp_unslash($_POST['display_subtitle'] ?? 'Un espacio para conectar, cuidar y celebrar mascotas.'));
    ...
    public static function get_settings(): array {
        ...
        return wp_parse_args($settings, [
            ...
            'display_name' => 'PetUnity',
            'display_subtitle' => 'Un espacio para conectar, cuidar y celebrar mascotas.',
        ]);
    }

    public static function get_frontend_branding(): array {
        $settings = self::get_settings();
        $name = sanitize_text_field((string) ($settings['display_name'] ?? 'PetUnity'));
        if ($name === '') {
            $name = 'PetUnity';
        }
        ...
        return [
            'name' => $name,
            'subtitle' => sanitize_text_field((string) ($settings['display_subtitle'] ?? 'Un espacio para conectar, cuidar y celebrar mascotas.')),
        ];
    }
